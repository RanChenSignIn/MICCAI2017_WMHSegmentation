...
config.gpu_options.per_process_gpu_memory_fraction = 0.9#设置gpu利用百分比
...
def get_unet(img_shape = None, first5=True):
        ...
        model.compile(optimizer=Adam(lr=(0.0002)), loss=dice_coef_loss)
        return model

def augmentation01(x_0, x_1, y):
...
def main():
    warnings.filterwarnings("ignore")
    # imgs_T2flair_T1_concate_Total = np.load('./imgs_T2flair_T1_concate_Total_npy.npy')
    # imgs_mask_concate_Total = np.load('./imgs_mask_concate_Total_npy.npy')
    ...
    train_leave_one_out(images, masks, patient=60, full=True, verbose=True)
